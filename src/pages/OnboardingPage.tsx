...
const CONDITIONS: { id: HealthCondition; label: string; description: string; icon: string }[] = [
  {
    id: 'hypertension',
    label: 'Hypertension',
    description: 'High blood pressure — track sodium & potassium',
    icon: '🫀',
  },
  {
    id: 'high_cholesterol',
    label: 'High Cholesterol',
    description: 'Monitor saturated fat, cholesterol & fiber',
    icon: '🩸',
  },
  {
    id: 'type2_diabetes',
    label: 'Type 2 Diabetes',
    description: 'Track carbohydrates, added sugar & fiber',
    icon: '📊',
  },
  {
    id: 'stroke_risk',
    label: 'Stroke Risk',
    description: 'Combined sodium, saturated fat & potassium tracking',
    icon: '🧠',
  },
]
      ...
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-white/50 text-sm mt-2">
            Select any conditions that apply. PlateIQ will flag meals that exceed clinical limits for each.
          </p>
        </div>
        ...
        >
          {selected.size > 0 ? `Continue with ${selected.size} condition${selected.size > 1 ? 's' : ''}` : 'Continue'}
        </button>
        <button
          onClick={handleSkip}
          className="w-full py-3 mt-2 text-white/40 hover:text-white/70 text-sm transition-colors"
        >
          Skip — I don't have any conditions
        </button>
      </div>
    </div>
  )
